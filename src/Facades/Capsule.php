...
use Nacosvel\RoosterServer\CapsuleManager;
...
class Capsule extends Facade
{
    ...
    {
        if (false === application()->getContainer()->has(DatabaseConfigInterface::class)) {
            application()->bind(DatabaseConfigInterface::class, function () {
                return require_once __DIR__ . '/../../config/rooster-server.php';
            });
        }
        if (false === application()->getContainer()->has(CapsuleManagerInterface::class)) {
            application()->bind(CapsuleManagerInterface::class, function () {
                $capsule     = new CapsuleManager();
                $flue        = new Fluent(application(DatabaseConfigInterface::class) ?: []);
                $key         = $flue->get('default');
                $connections = $flue->get('connections', []);
                if (array_key_exists($key, $connections)) {
                    $capsule->registerConnection($connections[$key], $key, $flue['schema'] ?? []);
                }
                $capsule->setAsGlobal();
        ...
        return CapsuleManagerInterface::class;
    }

    /**
     * Get a resolved facade instance.
     *
     * @return CapsuleManagerInterface
     */
    protected static function getFacadeInstance(): CapsuleManagerInterface
    {
        return application(CapsuleManagerInterface::class);
    }
